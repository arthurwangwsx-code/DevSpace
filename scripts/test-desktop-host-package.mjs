...
try {
  ...
  assert.equal(result.permissions?.version, "0.4.4");
  ...
  assert.equal(bridgeReplies[0]?.result?.serverInfo?.version, "0.4.4");
  assert.equal(bridgeReplies[1]?.result?.structuredContent?.version, "0.4.4");
  ...
  try {
    ...
    assert.equal(status.structuredContent?.version, "0.4.4");
  } finally {
