...
async function runControlAction(action: string, body: Record<string, unknown>): Promise<unknown> {
  ...
  switch (action) {
    ...
    case "update.install": {
      const { updateDevSpace } = await import("./update-manager.js");
      return updateDevSpace({ launchApp: false });
    }
    case "update.rollback": {
      const { rollbackDevSpace } = await import("./update-manager.js");
      return rollbackDevSpace({ launchApp: false });
    }
    default:
...
function controlCenterHtml(): string {
  ...
  </style></head><body><div class="shell"><aside class="sidebar"><div class="brand">DevSpace</div><div class="brandSub">Agent control plane</div><div class="nav"><button data-page="overview" class="active">Overview</button><button data-page="settings">Settings</button><button data-page="browser">Browser</button><button data-page="desktop">Computer Use</button><button data-page="updates">Updates</button></div><div id="sideStatus" class="sideStatus">Checking service…</div></aside><main class="main">
