...
| 45 | 长时压测原子进度回执完成（2026-09-13） | `442a582` + 本批文档提交 | Runner 发布串行、节流且单调的阶段进度；CLI 以原子 rename 持续写入 `progress.json`，包含 workload、进程树与持久状态摘要，并区分 running/complete/failed；进度回调不进入最终报告；typecheck、完整 `npm test`、build 通过 | 隔离 smoke 运行中成功读取 `run_workload:capacity`，完成后同一路径为 `complete:complete`、80/80、100%，JSON 全程可解析；功能门 26/26 通过，仅因提交前 runtime source 为 dirty 而按预期拒绝 release eligibility | 合入干净提交后需再跑 clean smoke/core release 形成 provenance 正向 receipt；当前已运行的 24h soak 不热升级、不重启，仍以原版本最终回执为准 |
