...
| 27 | 统一 release lane 与生产桌面 Fixture 框架完成（2026-09-13） | 本批提交 | `test:capability-release` 记录 commit、dirty paths、逐门日志和 JSON/Markdown receipt，按 core/locked/unlocked/browser-transition 编排且不把跳过或前置条件失败算作通过；`test:desktop-runtime-fixture` 覆盖固定 REST API→Runtime→动态 Provider→签名 Host→Fixture App，并验证 AX 脱敏、窗口截图、输入和应用重启后旧 lease 失效 | 锁屏下生产桌面 Fixture 在任何 UI 调用前按预期拒绝；release runner 的真实线上 reload 子门通过，完整 locked lane 待本批提交后执行 | unlocked 与 browser-transition lane 需解锁；24h soak 独立完成后才形成最终 release evidence set |
