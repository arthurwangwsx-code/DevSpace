...
| 29 | 最终 release 证据集校验框架完成（2026-09-13） | 本批提交 | stress report 增加 Git commit、全部 dirty paths、runtime-source dirty 子集及 `runtime_source_provenance` 硬门；`verify:capability-release` 要求显式 locked/unlocked/browser-transition/24h-soak 四份 receipt，逐门检查、比较 lane runtime 与当前 HEAD，并拒绝漏跑、失败、短于 24h、持久化越界、资源增长、孤儿进程或当前源码 WIP；旧 soak 必须显式留下 waiver | 待提交后用 smoke 生成带 provenance 的正向 receipt，并用现有不完整 evidence set 验证 fail-closed | 当前运行中的 24h soak 属 provenance 门加入前启动；最终可显式 legacy waiver，但不能隐藏该事实 |
| 30 | 浏览器重启恢复缺口修复并完成生产实链验证（2026-09-13） | `60ea716`、`c98ff78` + 本批真实 smoke 提交 | Native Host 隔离测试覆盖 Unix socket 断开/重建、双向消息、2 MiB 响应，并要求缓存的 `profile_hello` 在新连接恰好重放一次；浏览器能力契约升为 2.2.0，避免持久目录把 Schema/执行契约变化误判为同版本漂移，并为文件上传/下载等待提供长操作 deadline；锁屏转换矩阵增加 `--baseline-only`；真实 Profile smoke 对成功/失败均生成 JSON/Markdown receipt | 安装修复后的 Host 后，Chrome 无需重载即恢复同一 Profile 与 Extension 0.3.0；DevSpace 重启后 `browser.control` 自动恢复 ready；真实 canonical Browser smoke 连续 3 次 11/11，通过 Shadow DOM、同源 iframe、上传、network-idle、URL wait、下载与 lease cleanup；服务短暂不可达时另生成 FAIL receipt，证明 fail-closed | 完整 lock→unlock 矩阵可作为独立长稳态验证；日常发布前可运行无需人工锁屏的真实 Profile smoke |
| 31 | 浏览器 Fixture 有界清理完成（2026-09-13） | 本批提交 | lock matrix 与真实 Profile smoke 在关闭 loopback fixture 时立即回收 idle connection，1 秒后回收 fixture-only keep-alive，5 秒硬上限结束；typecheck 通过 | 修复前 baseline 的 13 个动作全部成功但 `server.close()` 被 Chrome keep-alive 挂住超过 3 分钟；修复后 baseline 13 步在 10.1 秒内 PASS，receipt `.build/browser-extension-baseline-current/2026-09-13T09-46-17-061Z`；真实 Profile smoke 11/11 在 4.2 秒内 PASS，receipt `.build/browser-control-real/2026-09-13T09-46-31-094Z` | 完整人工 lock→unlock 转换仍按用户决定后续执行 |
...
| 36 | 浏览器安全输入快照脱敏代码与验收门完成（2026-09-13） | 本批提交 | Extension snapshot 对 `input[type=password]` 不再读取 `value`，仅返回 `secure: true`；真实 Profile smoke 增加安全字段写入、页面侧长度确认、明文不出现在 snapshot 的强断言；Extension 单测、typecheck、完整 `npm test` 通过；发布材料升级为 0.3.1，doctor 报告安装与当前 Profile 配置就绪 | 当前 Chrome 仍运行旧 Service Worker，新增真实门在 `secure === true` 处按预期失败，receipt `.build/browser-control-real/2026-09-13T10-30-57-042Z`，证明测试能区分旧实现且失败不会被吞掉 | Chrome 安全策略禁止自动打开内部扩展管理页；需用户在 `chrome://extensions` 对 DevSpace Browser Bridge 点一次 Reload 后重跑真实 smoke，不能将 staged 0.3.1 冒充运行时已生效 |
| 37 | 生产 Workspace MCP 命令可用性硬门完成（2026-09-13） | 本批提交 | `devspace verify` 不再只校验工具数量，而是要求精确 Codex 工具集合，并通过公开 Schema 的 `yieldTimeMs` 实际执行 `open_workspace/read/exec_command`；locked、unlocked、browser-transition 三条生产 lane 都在昂贵测试前加入 `production_workspace_mcp_canary`；最终 evidence-set verifier 与门禁契约测试同步更新 | 本批实现前已在生产 `50292e3` 对 `/mcp` 完成真实 canary：6 tools、恢复既有 workspace、`exec_command` 退出码 0，总耗时 184.64 ms；Secure MCP Tunnel 同时为 live/ready 且 control-plane poll 正常 | 此门默认验证生产本地 MCP 数据面；远程 Tunnel 最后一跳仍由独立 tunnel health/control-plane canary 与外部 Agent 调用共同验收 |
| 38 | 生产命令面与同 Workspace 并发修复完成（2026-09-13） | `a711849`、`f34f3be`、`7d984e9` | production codex mode 固定暴露 `open_workspace/release_workspace/read/apply_patch/exec_command/write_stdin`；进程资源回归覆盖全局 4、单 Workspace 2 个并发命令槽；三条生产 lane 均以实际命令 canary 前置 | 同一 AiBox Workspace 中一个长时间 `swift test` 占槽时，第二个短命令约 11 ms 完成；本机 MCP 大文件分页、patch、命令及 workspace 恢复 smoke 通过 | 外部 Tunnel 的最终调用仍由实际外部 Agent 共同验收；单 Workspace 2 槽是有界并发，不是无限并发 |
| 39 | 稳定 TCC 身份与桌面生产动作闭环（2026-09-13） | `5e9d664`、`2c1c8e6`、`2abb424`、`ad496b9`、`901ca74`、`089b7ff` | LaunchServices stdio bridge、内部 PID/bundle 注入字段与公开 schema 分离、长生命周期 App 状态刷新、聚焦 AX 文本写入、fixture target 强持有及 lease expiry 回归进入完整测试；typecheck/build 通过 | 固定签名 Host 0.4.4、生产 REST→Runtime→Provider→Host 路径连续 3 次 22/22；最终 receipt `.build/desktop-runtime-fixture/2026-09-13T11-12-06-018Z`。含 reload 的性能门 REST/MCP p95 31/28 ms、Catalog 连续 286 次、RSS 增长 320 KiB，receipt `.build/desktop-provider-performance/2026-09-13T11-09-35-254Z` | 用户决定把完整人工 lock→unlock 矩阵留到后续；生产 fixture 只证明解锁态真实 AX/截图/输入与重启租约边界 |
| 40 | 生产服务可恢复自激活与发布门完成（2026-09-13） | `2ccb23a`、`fcdf791` | installer 单测要求调用返回前不执行 bootout，并验证 detached activator 的 bootout/bootstrap/enable/health 顺序；新增固定 `service-transition` release lane，最终 evidence-set verifier 强制要求该 receipt；完整 `npm test`、typecheck、build 与门禁契约通过 | 真实生产 MCP 内 `exec_command` 提交激活后返回；`service-transition` 3/3 PASS，PID `89369→90545`，恢复约 3.62 s、32 个不可达采样，随后新 MCP session 的 `pwd` canary 成功；receipt `.build/capability-release-current/2026-09-13T12-04-23-953Z`，Tunnel 最终 live/ready | 这是有界可恢复重启，不是零停机；计划切换期间现有 MCP session 需要重连/重试 |
| 41 | Provider 原子动态更新与生产 Schema 恢复完成（2026-09-13） | `f6b3e85`、`1dab9d5`、`f73bd1c` | 固定 REST 新增 `PUT /admin/providers/:id`，固定 MCP Catalog 新增 `devspace.providers.update`，外层仍为 8 tools；集成测试覆盖 ID 不匹配拒绝、版本更新、启动失败时 Manifest 与旧运行实例双回滚；真实 `chrome-devtools-mcp` 1.9.0 完成 install→update→disable→enable→reload→remove，29 项能力版本 `1.0.0→2.0.0`、Catalog revision `2→3→7`、子进程最终 0，receipt `.build/real-mcp-mount/2026-09-13T12-23-53-284Z` | 生产桌面旧 `2.1.0` Manifest 在重启后真实复现 `conflict`，随后经固定 MCP 动态更新到 lease 能力 `2.2.0`，Provider 恢复 ready；桌面 Fixture 22/22、reload 性能门 9/9；增强后的 service self-activation PID `78017→87632` 通过，当前提交 service-transition 3/3 releaseEligible，receipt `.build/capability-release-current/2026-09-13T12-22-51-096Z`，Tunnel 最终 live/ready | 当前 Chrome 仍需用户手动 Reload Extension 使 staged 0.3.1 生效；完整 lock→unlock 矩阵按用户决定后测；有界 24h soak 继续运行 |
| 42 | Browser lease 错误契约补齐（2026-09-13） | `3f29878` + 本批文档提交 | Browser Bridge 的 invoke/open/close 统一错误归一化；单测以 25 ms 无响应连接真实走过 lease open timeout，并验证断线 `provider_unavailable`、超时 `timeout`、取消 `cancelled`；变更后完整 `npm test` 与 typecheck 通过 | 生产真实 Chrome 在系统高负载时捕获一次 lease open 10 s 超时，旧代码返回 500，receipt `.build/browser-control-real/2026-09-13T12-28-06-918Z`；清理该 Agent 标签后立即重试，lease 成功且继续到安全字段版本门，receipt `.build/browser-control-real/2026-09-13T12-30-30-162Z` | Chrome 当前运行的 Extension 仍报告 0.3.0；需用户在扩展管理页手动 Reload 到 staged 0.3.1 后完成真实 smoke。锁屏矩阵按用户决定后测 |
| 43 | 浏览器 Native Host 竞态修复完成，生产部署待维护窗口（2026-09-13） | `8ad195b` + 本批文档提交 | 同 Profile 新连接和请求超时会通过 chmod-0600 Unix socket 下发本地 `native_host_shutdown`，Host 消费后退出且不把控制帧转发给 Chrome；250 ms fallback 有界回收旧 Host；Bridge/Host/Provider 定向测试、typecheck、完整 `npm test`、build 全通过 | Extension 0.3.1 已在用户当前 Chrome Profile 生效；修复前完成 canonical real smoke 12/12（`.build/browser-control-real/2026-09-13T12-51-28-631Z`）及外部 HTTPS 只读 7/7（`.build/browser-open-world-readonly/2026-09-13T12-52-11-988Z`），但本批 Host 退役协议尚未部署到生产 | 为避免再次制造 Tunnel 502，当前生产 PID 保持运行；待明确维护窗口安装新 Host/服务后，复验同 Profile 替换、超时恢复和真实 smoke。完整 lock→unlock 矩阵仍按用户决定后测 |
| 44 | Bridge 与 Native Host 组合竞态回归完成（2026-09-13） | `7ed1f91` + 本批文档提交 | 新测试启动真实 Native Host 子进程并连接真实 `BrowserExtensionBridge` Unix socket，完整验证同 Profile 替换、旧 Host 退出码 0、控制帧不进入 Chrome framing、替换 Host 接管、请求超时退役、Profile 清理和第三个 Host 恢复；独立命令进入 `pretest`；竞态重复 20/20、完整 `npm test`、typecheck、build 均通过且无遗留子进程 | 使用真实 Host 进程和 socket transport，但 Chrome 端由原生消息 framing harness 模拟；不替代用户当前 Profile 的生产复验 | 生产部署仍等待不干扰外部 Agent 的维护窗口；合入主分支前需等待图形化 APP Agent 完成当前 cherry-pick，不能混入其索引或解决其 README 冲突 |
| 45 | 长时压测原子进度回执完成（2026-09-13） | `442a582`、`ed65d1b` + 本批证据提交 | Runner 发布串行、节流且单调的阶段进度；CLI 以原子 rename 持续写入 `progress.json`，包含 workload、进程树与持久状态摘要，并区分 running/complete/failed；进度回调不进入最终报告；typecheck、完整 `npm test`、build 通过 | 隔离 smoke 运行中成功读取 `run_workload:capacity`，完成后同一路径为 `complete:complete`、80/80、100%，JSON 全程可解析；干净提交 smoke 26/26 通过，receipt `.build/capability-progress-clean/2026-09-13T13-31-15-567Z`；core release 5/5、`releaseEligible=true`，receipt `.build/capability-release-browser-host-progress/2026-09-13T13-31-39-444Z` | 当前已运行的 24h soak 不热升级、不重启，仍以原版本最终回执为准；本批提交仍在隔离 worktree，待图形化 APP Agent 清理主工作树 cherry-pick 后再安全合入 |
